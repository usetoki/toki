...
idempotency(api, { store: new MemcachedStore({ client }) });`,
    ...
    { kind: "heading", id: "options", text: "Options" },
    {
      ...
      rows: [
        ["`store`", "`IdempotencyStore`", "`MemoryStore()`", "Where reservations and records live."],
        ...
        ["`required`", "`boolean`", "`false`", "Reject participating methods with `400` if absent."],
        ["`ttl`", "`number`", "`86400`", "How long a completed response stays replayable, seconds."],
