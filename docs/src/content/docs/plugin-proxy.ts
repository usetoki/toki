    ...
    },
    {
      ...
      rows: [
        [
          ...
          "string",
          "(required)",
          "Upstream base URL, e.g. `http://internal:9000`. A crafted path can't redirect the proxy off this origin.",
        ],
        [
          "rewritePath",
          "(path: string) => string",
          "forward unchanged",
          "Map the incoming path to the upstream path. Query string is preserved separately.",
        ],
        ...
        [
          ...
          "string[]",
          "none",
          "Request headers to drop before forwarding (case-insensitive).",
        ],
        [
          "trustProxy",
          "boolean",
          "false",
          "Append to an inbound `X-Forwarded-For` and honor `X-Forwarded-Proto`. Off = overwrite with the real peer.",
        ],
        [
          "timeoutMs",
          "number",
          "none",
          "Abort the upstream call after this many ms, then return `502`.",
        ],
        [
          "fetch",
          "typeof fetch",
          "global fetch",
          "Inject a custom `fetch` (custom agent, mTLS, tests).",
        ],
      ],
    },
    {
