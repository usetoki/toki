...
// routes outside /dashboard have no req.session and no cookie work`,
    ...
    { kind: "heading", id: "options", text: "Options" },
    {
      ...
      rows: [
        ...
        ["`cookie.httpOnly`", "`boolean`", "`true`", "hide the cookie from `document.cookie`."],
        [
          "`cookie.sameSite`",
          '`"Strict" \\| "Lax" \\| "None"`',
          '`"Lax"`',
        ...
        ["`cookie.secure`", "`boolean`", "—", "only send over HTTPS; set it in production."],
        ["`cookie.domain`", "`string`", "—", "scope the cookie to a domain."],
        ["`cookie.partitioned`", "`boolean`", "—", "opt into partitioned (CHIPS) storage."],
      ],
    },
    {
      kind: "callout",
