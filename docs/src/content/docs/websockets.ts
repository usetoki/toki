  ...
  if (socket.protocol === "chat.v1") {
    ...
    { kind: "heading", id: "the-socket", text: "The connection" },
    {
      ...
      rows: [
        [
          "`socket.send(data)`",
          "Send a text (`string`) or binary (`Uint8Array`) message. Returns the write backlog in bytes — `0` once flushed, `-1` if the connection is closed.",
        ],
        [
          "`socket.ping(data?)`",
          "Send a ping; the peer answers with a pong. Payload capped at 125 bytes.",
        ],
        [
          "`socket.pong(data?)`",
          "Send an unsolicited pong, e.g. a one-way heartbeat. Payload capped at 125 bytes.",
        ],
        [
          "`socket.close(code?)`",
          "Send a close frame (default code `1000`) and tear the connection down.",
        ],
        ["`socket.protocol`", 'The negotiated subprotocol, or `""` when none was agreed.'],
        [
          "`socket.data`",
          "A free-form `Record` for per-connection state — a user id, a room name, a sequence counter.",
        ],
    ...
    { kind: "heading", id: "events", text: "Events" },
    {
      ...
      rows: [
        ...
        [
          ...
          "Once, with the peer's close code and reason.",
        ],
  ...
  maxWsMessageBytes: 4 * 1024 * 1024, // reject anything over 4 MiB
    ...
    },
    {
      ...
      rows: [
        ...
        [
          "`1007`",
          "Invalid payload — a compressed text frame that inflated to bad UTF-8 (sent for you).",
        ],
