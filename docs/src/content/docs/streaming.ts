...
app.listen(3000);`,
    ...
    {
      kind: "paragraph",
      text: "A Node `Readable` is an async iterable, so a file stream drops straight in. The file is read in chunks and pushed to the socket as it goes — memory stays flat no matter how large the file. For serving a directory of assets, prefer [static files](/docs/static-files), which reads once and serves natively; reach for a stream when the path is computed per request or the bytes come from elsewhere.",
    },
    {
      kind: "code",
      snippet: {
...
async function* ticker() {
  for (let n = 0; ; n++) {
    ...
    {
      kind: "paragraph",
      text: "Backpressure is native and automatic. Each chunk reports the connection's write backlog; once it passes 1 MiB, toki pauses the producer (it stops pulling from your iterator) until the socket drains. A slow client throttles your generator instead of blowing up memory — you write nothing special.",
    },
    {
      kind: "list",
      items: [
