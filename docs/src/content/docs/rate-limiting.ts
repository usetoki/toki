  ...
  rateLimit: { max: 100, windowMs: 60_000 }, // 100 requests / minute / IP
    ...
    { kind: "heading", id: "how", text: "How it works" },
    {
      ...
      items: [
        ...
        "The key is the peer IP. Behind a proxy the limiter sees the proxy's IP — terminate the limit at the edge, or raise `max` to fit your fan-out.",
    ...
    {
      kind: "paragraph",
      text: "When you need a custom key (an API key, a user id) or per-route limits, write a middleware. A middleware that returns a response short-circuits the request — return `reply.json(..., 429)` to reject. This runs in JS, so reserve it for the routes that need it; keep the native limiter as the outer flood guard.",
    },
    {
      kind: "code",
      snippet: {
