...
const exceeded = mod.exceeded;
const init = mod.init;
const reset = mod.reset;
const retryAfterSeconds = mod.retryAfterSeconds;
const sweep = mod.sweep;
...
test "sweep removes elapsed buckets" {
    ...
    try std.testing.expectEqual(@as(u32, 1), mod.table.count());
    sweep(2000);
    try std.testing.expectEqual(@as(u32, 0), mod.table.count());
}
...
test "reset frees buckets and disables" {
    ...
    try std.testing.expectEqual(@as(u32, 2), mod.table.count());
    reset();
    // limiter disabled and every bucket freed (testing.allocator catches leaks)
    try std.testing.expectEqual(@as(u32, 0), mod.table.count());
    try std.testing.expectEqual(@as(u32, 0), mod.max);
    try std.testing.expectEqual(@as(u64, 0), mod.window_ms);
    try std.testing.expect(!enabled());
}
...
test "boundary at exactly window_ms opens a fresh window" {
    ...
    try std.testing.expect(!exceeded("8.8.8.8", 1000)); // elapsed == mod.window_ms → reset
...
test "rejected request does not bump the count" {
    ...
    // window resets cleanly; count was held at mod.max, not inflated
...
test "sweep is a no-op when disabled" {
    ...
    mod.max = 0; // disabled
    mod.window_ms = 1000;
    // populate directly to prove sweep leaves a disabled mod.table untouched
    const key = try std.testing.allocator.dupe(u8, "15.1.1.1");
    try mod.table.put(std.testing.allocator, key, .{ .count = 1, .window_start = 0 });
...
test "sweep keeps live buckets and drops only expired" {
    ...
    try std.testing.expect(mod.table.contains("16.0.0.2"));
    try std.testing.expect(!mod.table.contains("16.0.0.1"));
}
...
test "max u32 boundary never rejects in practice" {
    init(std.testing.allocator);
    defer reset();
    mod.max = std.math.maxInt(u32);
    mod.window_ms = 1000;
    var i: u32 = 0;
    while (i < 1000) : (i += 1) {
        try std.testing.expect(!exceeded("19.1.1.1", 0));
    }
    try std.testing.expectEqual(@as(u32, 1000), mod.table.get("19.1.1.1").?.count);
}
...
test "zero window allows every request" {
    ...
    defer reset();
    mod.max = 1;
    mod.window_ms = 0; // elapsed >= 0 is always true → bucket resets every call
    ...
    try std.testing.expectEqual(@as(u32, 1), mod.table.get("20.1.1.1").?.count);
}
...
test "interleaved ips keep independent counts" {
    ...
    try std.testing.expect(exceeded("a", 2)); // a hit its mod.max
    try std.testing.expect(exceeded("b", 2)); // b hit its mod.max
    try std.testing.expectEqual(@as(u32, 2), mod.table.count());
}
...
test "many distinct ips each get a bucket" {
    ...
    try std.testing.expectEqual(@as(u32, 256), mod.table.count());
}
